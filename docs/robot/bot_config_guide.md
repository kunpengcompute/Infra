...
本文档面向**社区开发者**，说明如何通过配置文件自助调整机器人在你的仓库中的行为，无需联系平台管理员。
...
|--------|------|-------------|----------|--------|
| 1（最高优先级） | 仓库个性配置 | 你的仓库根目录 `.infra/robot.yaml` | 仅对本仓库生效，按 PR 目标分支隔离 | 仓库维护者 |
...
在你的仓库**根目录**创建 `.infra/robot.yaml` 文件，所有可自定义的功能配置均写在这一个文件中。

### 2.2 按分支隔离

机器人在处理每个 PR 时，以该 **PR 的目标分支**为准读取配置文件。即：

- 合入 `main` 分支的 PR → 读取 `main` 分支根目录的 `.infra/robot.yaml`
- 合入 `release-1.0` 分支的 PR → 读取 `release-1.0` 分支根目录的 `.infra/robot.yaml`
...
组织配置由**组织管理员**统一维护，存放在组织内 `infrastructure` 仓库的 `.infra/robot.yaml` 文件中。
...
2. 文件名是否为 `.infra/robot.yaml`，路径是否在仓库根目录。
